...
class InsatienFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        foreach ($data as $i => [$nom, $prenom, $email, $promoYear, $parcoursId]) {
            $insatien = new Insatien();
            $insatien->setNom($nom);
            $insatien->setPrenom($prenom);
            $insatien->setEmail($email);
            $insatien->setPromoYear($promoYear);
            $insatien->setParcours($manager->getRepository(Parcours::class)->find($parcoursId));
            $insatien->setFiliere($filieres[array_rand($filieres)]);

            $manager->persist($insatien);
            $this->addReference('insatien_' . ($i + 1), $insatien);
        }
